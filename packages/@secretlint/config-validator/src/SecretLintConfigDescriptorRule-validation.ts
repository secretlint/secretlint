...
export const isSecretLintConfigDescriptorRule = ajv.compile(SecretLintConfigDescriptorRuleSchema) as ValidateFunction<
...
export default function validate(value: unknown): SecretLintConfigDescriptorRule {
    if (isSecretLintConfigDescriptorRule(value)) {
        ...
        throw new Error(
            ajv.errorsText(
                isSecretLintConfigDescriptorRule.errors!.filter((e: any) => e.keyword !== "if"),
                { dataVar: "SecretLintConfigDescriptorRule" }
            ) +
                "\n\n" +
                inspect(value)
        );
